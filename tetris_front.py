...
class TetrisGui(tk.Tk):
    ...
    def start_game(self):
        if self.scheduled_step is not None:
            self.after_cancel(self.scheduled_step)

        self.back.reset()
        self.color_grid.delete(tk.ALL)

        y_limit = self.back.starting_row * self.factor
        y_floor = self.back.height * self.factor
        self.color_grid.create_line(0, y_limit, self.back.width * self.factor, y_limit)
        for j in range(self.back.width):
            x = j * self.factor
            self.color_grid.create_line(x, y_limit, x, y_floor)
